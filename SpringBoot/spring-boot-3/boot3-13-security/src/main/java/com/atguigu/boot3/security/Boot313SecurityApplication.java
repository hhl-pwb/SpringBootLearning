...
/**
 * Security 场景的自动配置类：
 * SecurityAutoConfiguration、SecurityFilterAutoConfiguration、SpringBootWebSecurityConfiguration
 * 1、security的所有配置在 SecurityProperties：以spring.security开投
 * 2、默认SecurityFilterChain组件：
 * 		- 所有的请求都需要认证（登录）
 * 		- 开启表单登录:spring security提供一个默认登录页，未经登录的所有请求都需要登录
 * 		- httpbasic方式登录
 * 3、@EnableWebSecurity生效
